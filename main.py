...
def find_closest_note(pitch):
  ...
  """
    i = int(np.round(np.log2(pitch / CONCERT_PITCH) * 12))  # pitch = note detected, concert pitch = accurate pitch
    closest_note = ALL_NOTES[i % 12] + str(4 + (i + 9) // 12)  # Formulation of closest note
...
def callback(indata, frames, time, status):
  ...
  """
    ...
    if not hasattr(callback, "window_samples"):  # hasattr = has attribute -> is a function, callback is a class while window samples is a variable
